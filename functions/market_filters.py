...
EVENT_POKEMON = {
    "event": [
        "Boss Squirtle", "Cupcake Swirlix", "Balloons Spheal", "Gifts Vulpix"
    ]
}
...
def filter_stats(pokemon, filters):
    print(f"Checking Pokémon {pokemon['pokemon_name']} with stats: {dict(pokemon)}")  # Debugging line
    ...
    for stat, (op, value) in filters.get("stats", {}).items():
        ...
        if stat_column in pokemon:
            ...
            print(f"Stat {stat_column} not found in {pokemon['pokemon_name']}!")  # Debugging line
            return False  # If stat is missing, it fails the filter

        print(f"Checking {stat_column}: {pokemon_stat_iv} {op} {value}")  # Debugging line
...
async def filter_fusion(user_id, db_pool):
    query = """
    SELECT pokemon_name FROM market
    WHERE fusionable = TRUE
    """
